...
public class LightSensorSource extends AbstractSensorSource {
    ...
    public void init(SourceEventListener sourceEventListener, OptionHolder optionHolder, String[] strings, ConfigReader configReader, SiddhiAppContext siddhiAppContext) {
        ...
        if (sensor == null) {
            ...
            throw new SiddhiAppCreationException("Light Sensor is not supported in the device. Stream " + sourceEventListener.getStreamDefinition().getId());
        }
    }
    @Override
    public void onSensorChanged(SensorEvent event) {
        ...
        if (this.pollingInterval == 0L && (this.latestInput == null || (float)this.latestInput.get("light") != (float)output.get("light"))) {
